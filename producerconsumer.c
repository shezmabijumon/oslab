#include <pthread.h>
#include <semaphore.h>
#include <stdio.h>
#include <stdlib.h>
# define MaxItems 5 //maximum items a producer can produce or a consumer can consume
# define BufferSize 5 // Size of the buffer
sem_t empty;
sem_t full;
int in = 0;
int out = 0;
int buffer[BufferSize];
// declare a mutex variable
pthread_mutex_t mutex;

void *producer(void *pno){
	int item;
	for(int i = 0; i < MaxItems; i++) {
		// Produce an random item
		item = rand();
		//To lock a semaphore or wait 
		sem_wait(&empty);
		//Locking the critical section
		pthread_mutex_lock(&mutex);
		buffer[in] = item;
		printf("Producer %d: Produced Item %d at %d\n", *((int *)pno),buffer[in],in);
		in = (in+1)%BufferSize;
		// unlocking the critical section
		pthread_mutex_unlock(&mutex);
		// To release or signal a semaphore
		sem_post(&full);
	}

}

void *consumer(void *cno)
{
	for(int i = 0; i < MaxItems; i++) {
		sem_wait(&full);
		pthread_mutex_lock(&mutex);
		int item = buffer[out];
		printf("Consumer %d: Consumed Item %d from %d\n",*((int *)cno), item, out);
		out = (out+1)%BufferSize;
		pthread_mutex_unlock(&mutex);
		sem_post(&empty);
	}
}

int main()
{
	// Declaring thread variables for producer and consumer(here we use array for each...) 
	pthread_t pro[5], con[5];

	
	pthread_mutex_init(&mutex, NULL);
	//to initialize a semaphore ... sem_init(address of semaphore, if it is shared between processes, inital value)
	sem_init(&empty,0,BufferSize);
	sem_init(&full,0,0);
	int a[5] = {1,2,3,4,5}; //Just used for numbering the producer and consumer
	for(int i = 0; i < 5; i++) {
		// A thread is created and started
		pthread_create(&pro[i], NULL, (void *)producer, (void *)&a[i]);
	}
	for(int i = 0; i < 5; i++) {
		pthread_create(&con[i], NULL, (void *)consumer, (void *)&a[i]);
	}

	for(int i = 0; i < 5; i++) {
		// to set parent thread to wait for child threads
		pthread_join(pro[i], NULL);
	}

	for(int i = 0; i < 5; i++) {
		pthread_join(con[i], NULL);
	}
	pthread_mutex_destroy(&mutex);
	sem_destroy(&empty);
	sem_destroy(&full);
	return 0;
}